...
func (r *TodoListPostgres) Create(userId int, list model.TodoList) (int, error) {
	...
	var id int
	createListQuery := fmt.Sprintf("INSERT INTO %s (tittle, description) VALUES ($1, $2) RETURNING  id", todoListsTable)
	row := tx.QueryRow(createListQuery, list.Tittle, list.Description)
